...
testBuildCorpusFC trainingImages =
    ...
    in assess "buildCorpus implementation details" 4 $ do
        ...
        lin <- check "that buildCorpus is linear" $ buildCorpus `shouldBeLinearIn` (`take` (cycle trainingImages))
        return $ do
...
testSmoothing =
    ...
    in assess "Tests for smoothing" 7 $ do
        ...
        nOne <- checkLoud "that no probabilities = 1" $  results `shouldNotContain` 1
        sumOne <- checkLoud "that probabilities for a feature sum to one" $ zipWith (+) features complements `shouldBe` [1,1,1,1,1]
        noColl <- checkLoud "that there are no collisions" $
             (features `shouldSatisfy` unique) <> (complements `shouldSatisfy` unique)
        evenGap <- checkLoud "that gaps between consecutive probability estimates are even" $ 
...
testAvoidWork trainingData images = 
  ...
  in assess "that you avoid unnecessary work" 10 $ do
        check "that you've updated Corpus definition" $ "Corpus" `shouldNotBeType` "type Corpus = [(Digit, [PixelImage])]"
        check "that your code is constant in the number of training images" $ classifyImages `shouldBeConstantIn` numTrainingImages
